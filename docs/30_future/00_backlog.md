...
> **Pri 1–30**: B6 0 rows, B7 4 (1 L, 2 M, 1 S), B8 5, B9 1, B2 8, C 8 *(recounted 2026-09-21 at the #275 re-cut under note 64)*.
...
| 3 | **Appendix G integrates the fuselage nose to tail through the carry-through, where the model has no beam, and the wing through its side-of-body root** — the five M4-1 carry stations route to the nearer spar post and cross its cut, a 183 % "lumping" artifact on `concept_regional_jet` (83,887 lb against a 45,878 lb peak) and 111 % on `baron_58`, while the ATR's 43 % hides a cancelling pair at its rear post; the wing's worst shear term is the SOB cut on all four loaded fixtures, the inboard strips counted on the node the station curve excludes. The joint itself is ruled wrong in kind: the box between the spars is not a beam region *(note 56 §7b finding 3, measured 2026-09-12; re-cut 2026-09-21 under design note 64, AGREED the same day)* (#275) | Design note 64: the rigid centre-box hub and two spar posts become **one vertical post** at the wing station; the fuselage beam runs through the box with grids at both spars and the wing station; the wing beam continues straight across to BL 0; the body is integrated as **two cantilevers**, shear and bending positive for up load, the box's loads applied and never integrated; M4-1's linear smear and the closure-artifact fallback retire for one reaction at the wing station; Appendix G integrates as the deck does. Gates 1–11 of the note on every CLI-exportable fixture | V | **L** / L | design note 64 at AGREED (2026-09-21); **before #283**, which draws the joint this changes |
