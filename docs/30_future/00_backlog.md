...
> **Pri 1–30**: B6 0 rows, B7 3 (0 L, 2 M, 1 S), B8 5, B9 1, B2 8, C 8 *(recounted 2026-09-21 when #293 was filed at the #260 closure)*.
...
| 5 | **A centreline mass closure lands on one main-gear grid of a mirrored pair** — `transferred_case_loads` sends every balanced load to its nearest grid and `nearest_node` breaks ties to the first in order (CR-B-1, chosen for byte-stable decks); the main-gear item and the solved ballast row sit at waterline 55–60, nearer the ±30 in gear attach pair than the body beam, so their whole closure goes to the right grid: `atr42_100` LG-01 legs read 19,566 / 27,142 lb vertical on a case whose two reactions are both 27,142 (2,800 lb apart before #260, 7,576 after; found 2026-09-21 at the #260 cut-side-sum failure). First-order on every landing subcase of the deck and on the box reaction path; the deck still closes globally so no equilibrium gate saw it (#293) | A mass-closure load transferred to beam grids only, never to a gear attach grid (or the equidistant pair split half each), guarded by a mirrored-leg identity on every body-gear fixture; the ATR's `sbeam/*` digests move | V | M / S | before #161 (the baseline wave regenerates what it moves) |
