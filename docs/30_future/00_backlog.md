...
> filed as **#289** behind #288 (same shared ordinal); #260's E6 and the fixtures' hand-entered `wing_mass.cases` wait on it.
...
| 1 | **One mass model: the case's loading is the mass state of every inertia load** — WINGINER/NETLOADS distribute a second, project-wide mass list (`wing_mass.panel_weight_lb` + `concentrated`) that ignores the case, the balanced deck reads the winning V-n point's CG case through a *searched* loading (`loading` is `None` on every FLIGHT case of every fixture), and `body_loads` lumps the whole database; on `atr42_100` the −1.00 g minimum-weight down-bending pick carries full wing fuel in the net loads and none in the deck, the governing MTOW wing bending is **21 % high** and every zero-fuel case **11–18 % low** *(measured 2026-09-17, note 63 §8.4)*. Design note 63 (AGREED 2026-09-17, reviewed same day; §9 records the three review findings and their rulings — `body_loads` per case is a stated correction on the GA6, the slot id / run key split of D-63.11 keeps one id per condition under D-63.7, the v67 migration drops `concentrated` where the tie closes and stamps POINT) keeps one mass model — the item database and the case's D-25 loading — moves the wing's mass fields off the Wing Loads page (the fuselage precedent, note 50), and has every inertia consumer read the case's loading; **this is the one-model step of the R-63.4 scope split (ruled 2026-09-17): D-63.1…D-63.4, D-63.6, D-63.8, D-63.10, D-63.11 and every v67 field (`carriage`, the panel override, MZFW as a stored design weight, `CaseRef.run`/`config`); the MZFW seeds and the slot × case variants (D-63.5, D-63.7) are #292** (#289) | `MassItem.carriage`, `WingLoadCase.cg`, `weight.max_zero_fuel_weight_lb` (stored, seeds nothing until #292), `CaseRef.run`/`config`, `WingMassInput` without mass (v67 migration: drop where the tie closes, stamp POINT off-centreline; ATR and concept_heavy fixtures hand-corrected per D-63.4), per-case WINGINER/NETLOADS/`body_loads`; gate 1 (Appendix A byte-identical, `body_loads` corrected by the stated amount), G-63.1, G-63.3a's identity half, G-63.4, G-63.5, G-63.6's dictionary half; one digest wave; the variants split to #292, the loading editor to #290 | V | L / M | note 63 at AGREED; lands **after #288** (the slots the variants multiply); closes #257's reconciliation family by construction |
| 1 | **Each SELECT wing slot runs at every FLIGHT mass state and the net-governing variant is the delivered case** — today every wing slot is delivered at the mass state of its own V-n point's CG case, so a slot whose air pick sits at MTOW is never assessed at zero fuel, where the wing fuel's inertia relief is gone and root bending is highest; on `atr42_100` the zero-fuel PHAA root Mxx exceeds the MTOW pick's by the relief of 1,900 lb of fuel per side *(measured 2026-09-17, note 63 §8.4)*. Design note 63 D-63.5 and D-63.7 (AGREED 2026-09-17; the variants step of the R-63.4 scope split, ruled 2026-09-17): MZFW seeds three named FLIGHT cases with their loadings (`"mzfw aft"`, `"mzfw fwd"`, `"full fuel aft"`); the Wing Loads step expands the ten note 62 slots × the FLIGHT cases regardless of `wing_mass.cases` (an entered table is a filter on which slots run, never a second source of points), each variant is that family's own winning V-n point balanced at case *k* with *k*'s loading, the governing variant per slot is the extreme signed root Mxx at the loads reference axis (largest for the positive slots, most negative for the negative ones), and SELECT's delivered `CriticalCondition` is re-pointed to the governing run before the index, deck and report read it — `select.air_picks` keeps the per-family air pick as the queryable intermediate the Appendix A test asserts (#292) | The three MZFW seed cases written with their loadings (`seed_flight_cases`, `FLIGHT_CASE_NAMES` and its guard test; the heaviest-zero-fuel search objective); the slot × FLIGHT-case variant table in the Wing Loads step with the governing variant carrying the slot's W-nn id and run key (D-63.11's `CaseRef.run`/`config`, shipped by #289); the balanced deck's subcase mass set is the governing run's loading; the report's wing table lists every variant with the governing one marked; G-63.2 (relief sign on the ATR and Baron), G-63.3 and G-63.3a (table complete, one governing per slot, `$` header and CONM2 set name the same run, GA6 governing = air pick on every slot); one digest wave (`select`, `wing_inertia`, `net_loads`, balance, deck, report); `PROGRAM_SPEC.md` WINGINER/NETLOADS/SELECT, `ch04_wing_loads.md` variant table, `04_far25_gap_analysis.md` 25.321 to A; note 63 to SHIPPED | V | L / M | **directly after #289** (the one-model step: per-case WINGINER/NETLOADS, `carriage`, the run key and the v67 schema it reads), before #290; the fixtures' hand-entered `wing_mass.cases` (#260 E6) become filters here |
| 1 | **Payload Cases tab edits the case loading** — D-25 shipped `CgCase.loading` with no page that edits it, so a loading is typed into the JSON or not at all and every FLIGHT case on every fixture has none; note 63 D-63.9, split from #289 by the owner on 2026-09-17 so the physics PR stays reviewable (#290) | per case: aboard checkboxes over the discretionary rows, a fraction on each consumable row aboard, an optional ballast row, the D-25a echo; the Wing Loads page shows the per-case WING parts read-only; the `case_loading_missing` warning ships here, not with #289; field-registry totality and a GUI journey gate | V | M / M | after #289 (its schema) and **#292** (the variant table its read-only WING-parts view reflects) |
| 1 | **GA6 fixture: altitude identity + wing-case envelope, one package** — every delivered case states 0 ft where Appendix A names its critical wing conditions at 12,000 ft, and the wing export ships three cases with no negative-g, so the delivered distributions do not envelop the wing; each fix renumbers the V-n indices the other depends on, so this row is #164 **and #165 merged** — the renumber is paid once *(review R-26)* (#164) | Stated condition identities correct and the wing enveloped; the three oracle cases kept; oracle-locked fixtures renumbered in one pass | V | L / M | #289 (note 63) ships first (#288, the case-set ruling of note 62, shipped 2026-09-17); the wing-case envelope half is then the slot × case variant table (D-63.7), on which every GA6 governing variant is its Appendix A point (note 63 §8.2) |
...
| 5 | **`atr42_100` is internally inconsistent** — planform 18 % under the type's area with the true value stored-but-ignored, wing items 3.4x the panel integration, fuselage stations 3,741 lb short, five override warnings on load, no control-surface slices, **and one entered wing case** — `wing_mass.cases` holds PHAA alone with a zero couple, so the WINGINER distributions and the report's wing section run no negative-g, TORS or ACRL case on the ATR while the deck's six come from SELECT unaffected *(2026-09-09 review §4; E6 added 2026-09-16 from the 0.8.6 review)* (#260) | The fixture reconciled end-to-end from published ATR 42-300 data, its wing-case set shaped by the #164 ruling; in the CI matrices, so baselines move *(the baseline wave; #261 shipped 2026-09-10)* | V | M / M | the baseline wave, now **#164 → #222 → #161** — #241/#242 left it for 0.8.4 at the 2026-09-11 re-cut; the case-set shape follows note 62 (shipped 2026-09-17) and note 63 (#289, D-63.7) |
| 5 | **`concept_heavy`'s polar carries a forward non-wing axial force inside the trusted window — a fixture aero-data defect** — with NMAA narrowed to the VC pair (#288, design note 62 D-62.1) the heavy's NMAA is MAN −C at −2.0 g and trim alpha −9.8°, 0.2° inside the polar's trusted window, and there the airplane-less-tail polar less the wing strips comes out **forward** (dCD +0.0169), which inside the window can only mean the fixture's crude polar under-reads the airplane's drag at negative alpha; the STALL −N point the slot held before sat at −14.3°, outside the window, where design note 20 D-4 clamps the force and the defect was invisible. Recorded with its number in `tests/test_balance.py::_DELTA_CD_FORWARD_INSIDE_WINDOW`, asserted both ways, so it cannot outlive the defect *(found 2026-09-17 shipping #288; note 62 §8.3)* (#291) | The heavy's polar coefficients re-entered so that every case inside the trusted window has dCD < 0; the `_DELTA_CD_FORWARD_INSIDE_WINDOW` entry removed; `_DELTA_CD_BAND` and any clamp/ratchet the heavy's cases move re-pinned with the cause stated; one digest regeneration for the heavy's `balance` channels | V | S / S | #288 (shipped 2026-09-17); fixture-data only, no physics or schema change |
