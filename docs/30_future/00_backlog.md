...
| 1 | **The NNZ load-factor extreme is delivered by no wing slot on `baron_58` and `concept_regional_jet`** — `select_wing` applies the D-62.8 coincidence rule to the *air* picks before the D-63.7 re-pointing, so when NMAA re-points to another mass state the NNZ point it coincided with is carried by nothing; the PNZ/NNZ tie band is `TIE_REL` (1e-9) against a balance that converges `nz` to ±0.005, so the resultant tie-break never fires; and `AIR_PICK_SLOTS` is honoured only when the air pick survived *(0.8.6 pre-cut review, 2026-09-22, CRITICAL + two MAJOR)* (#294) | `wing_slot_picks(..., coincide=False)` for the re-pointing pass and one coincidence pass on the delivered set; an absolute `nz` tie band at the envelope's own convergence tolerance, owned once; the `AIR_PICK_SLOTS` test hoisted out of the `if air is not None` guard; a loss-side gate — every eligible PNZ/NNZ extreme is carried by some delivered W case — and G-62.2 re-pinned | V | M / S | — (blocks the 0.8.6 cut) |
| 2 | **The ACRL variant's root `Mxx` omits `unbal_moment`, so its governing run is chosen on a load that is not the load delivered** — `wing_variants.py:162` builds `WingLoadCase(name, case, nz, nx)` with the aileron couple at 0, and `mxx_r` varies per case with the point masses, so the omitted term is not constant across the compared rows; the Baron fragment lists ACRL among the slots that moved *(0.8.6 pre-cut review, 2026-09-22)* (#295) | The slot's entered couple carried into the variant, or a stated and gated rule that ACRL compares on the balanced part alone | V | S / S | — |
| 3 | **`_hop_66` converts a centreline `concentrated` entry to a POINT row and then stamps it PANEL** — `migrations.py:364` appends `{y: 0, weight: 2w, carriage: point}` and `_stamp` (:382, rule *point iff wing and y ≠ 0*) runs over every row afterwards, so the mass leaves the point list and a spurious `panel_weight_override_lb` is written; only the ±y branch is tested (`tests/test_migrations.py:435`) *(0.8.6 pre-cut review, 2026-09-22)* (#296) | Stamp before the conversion block (or only the pre-existing rows); the centreline case added to the migration test | V | S / S | — |
| 4 | **A wing station outside its spars raises a bare `KeyError` from the LRA exporter, not the `LraRefusal` note 64 gate 9 names** — `export/lra_model.py:965` calls `reg.one(JointName.WING_POST)` unguarded; `KeyError` is not a `ValueError`, so it escapes `lra_import.py:153`, `report/lumping.py`'s handler and the CLI error contract; gate 9 (`tests/test_body_loads.py:184`) covers the calc side only *(0.8.6 pre-cut review, 2026-09-22)* (#297) | The exporter guarded with `_refusal_reason(reg, JointName.WING_POST)` like every other missing-datum path; gate 9 extended to the exporter | V | S / S | — |
| 5 | **SI precision rows for area, moment and inertia copy the Imperial decimal count, delivering one or two significant figures** — `units.py:845` gives `m^2`, `N·m` and `kg·m²` the Imperial row's 0 decimals; `format_value(2.899, "m^2")` prints `3` (a 31.2 sq ft tail, −3.4 %); D-65.5's "within a factor of 2.2" is false for ft²→m² (10.8×) and lb-in²→kg·m² (3418×); `tests/test_platform_stability.py:472` asserts SI row == Imperial row and enshrines it *(0.8.6 pre-cut review, 2026-09-22)* (#298) | SI rows with decimals scaled by the conversion factor (or a minimum significant-figure floor); the equality assertion replaced by "no coarser than the source"; note 65 §7b amended | V | S / S | — |
...
| 9 | **A derived ACRL assembles with a zero aileron couple and the deck states nothing in band** — a reader cannot tell couple-free physics from couldn't-compute-the-couple *(2026-09-09 review A5; moved from report polish 2026-09-22 — a statement gap in the one deck that ships is #284's class, not polish)* (#258) | The zero-by-derivation stated in the case's notes, deck header and UI, like the ASSUMED thrust line; one guard test | V | S / S | — |
