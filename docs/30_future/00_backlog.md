...
| 2 | **oracle_app results captions still claim ULTIMATE over LIMIT tables** — the frozen half of #192; the understrength direction. **Moved B6 → B5 by design note 60 §5:** note 57's gates 4 and 5 are enforced by `tests/test_basis_statements.py`, whose `_GUI_TREES` excludes `oracle_app` — including it *is* this row, so the band whose gates assert through it cannot follow it *(2026-09-08 review G1)* (#239) | `_ULT_NOTE`/the ULTIMATE default replaced with the LIMIT statement; `oracle_app` added to `_GUI_TREES` so G-OR-74's screen sweep covers this front-end; the two `_GUI_TREES` owners (`test_basis_statements.py`, `test_app_shell.py`) converge on one under rule 4 *(note 60 §3)* | V | S / S | the 0.8.3 cut; **before #266**, whose gates assert through it |
...
| 4 | **Twenty figures port under one owner — `PlotData` producers, two renderers** — the report's figure data stays the single owner (`content.PlotData`, already shared by both documents); `plots_tex` emits LaTeX, a new `app_shell/plots.py` emits Plotly, and a both-ways parity guard holds them together. D-57.4's four-figure list and its *written fresh* ruling are **withdrawn**: thirteen of the 22 pages carry a chart at **twenty** call sites, nine of those pages are shared analysis steps the survivor renders today with no figure, and deriving the GUI's figures separately would build the second figure owner #239's drift class is made of *(design note 60 D-60.1…D-60.6, amending note 57 D-57.4)* (#267) | Every figure the oracle report carries rendered in the GUI **by construction**; each producer callable from a `Project` (pre-run) or from the module results (post-run) and classified as one, the extraction out of `oracle_sections.py` being the bulk of the row; `_vn_figure` promoted public; the figure gate and the parity guard in CI (note 60 §5, gates 9/10); the trim \& stability **sweep** stays deferred, its two figures port | V | **L / L** | after #266 |
| 5 | **The fleet comparison ports** — the Phase-C "assess vs similar airplanes" requirement lands with the extension tier it belongs to; `_subject_from_project` (CC E(34)) rewritten, not imported *(note 57 D-57.5)* (#268) | The fleet plots a page of the surviving GUI over `reference_aircraft.csv` unchanged | V | S / S–M | beside #267 |
| 6 | **The seed button is built fresh with its hardening** — seeded rows loudly incomplete until positioned and tagged; merge/refuse/replace stated before the click; port-then-harden rejected under rule 4 *(note 57 D-57.7; re-scopes #78)* (#269) | The WTESTIMA seed action in the surviving GUI with #78's hardening in its first version; #78 closes superseded when this lands | V | S / S | after #266 |
| 7 | **Applied-load CSVs drop the case identity: 33 gear cases collapse onto 8 Case strings** — `AppliedLoad.case_id` populated and never emitted *(2026-09-08 review C1)* (#241) | A `Case ID` column (+ `Loading` on the gear file) in `applied_load_csv`; round-trip assert into the case index; digest re-pin. **Note 56 landed 2026-09-12** — D-56.1 moved the `AppliedLoad` model rather than deleting it, so the fix lands at `sloads/report/applied.py` (`_applied_csv_fields`, `applied_load_csv`); the six `*_applied_loads.csv` files are now the *only* applied-load CSVs, the deck companions having gone with the decks | V | M / S | the 0.8.3 cut; **before #245**, so `data/` is born with the case identity |
| 8 | **No delivered CSV states the axis directions**, and two files make frame claims their data does not honour *(2026-09-08 review C2+C3)* (#242) | An AXES stanza in the methods stamp (one owner, every file); the control-surface `Fz` → surface-normal label; one meaning for `Axis`. **Narrowed by note 56, landed 2026-09-12** — the deck-companion span/chordwise/fitting CSVs it also covered went with their decks; the surviving set is the report's own (`report/applied.py`'s six `*_applied_loads.csv`, `report/tables.py`'s case-index / safety-factor / gear files, `io.py`'s `load_cases_csv`), and the control-surface `Fz` question survives as rows *inside* the h-tail and fin applied files rather than as a file of its own | V | S / S | the 0.8.3 cut; **before #245** |
| 9 | **The issue package's `data/` becomes the oracle GUI's only CSV channel** — **re-milestoned 0.8.3 → 0.8.4 at the 2026-09-11 re-cut** — it is the successor channel note 57 §1.3 names for `export_report`, so #270 cannot retire that page until it lands; pulled out of 0.9.0 into 0.8.3 by owner ruling (2026-09-09): implemented by consolidation, the per-module CSV/text buttons and the results zip retiring in the same change, preceded by the column-inventory pass (OEI march and V-n matrix named) (#245) | `data/` in every package build from the report's own owners, manifest rows, the intermediate channels gone, G-OR-73 re-cut to the consolidated set. **Note 56 does not supersede this row — ruling 6 *is* its scope.** #263 narrowed the column inventory instead (landed 2026-09-12): the deck-companion span/chordwise/fitting CSVs went with their decks, so `data/` need only carry what the document draws. **Confirmed and widened by design note 60 D-60.12 (2026-09-13):** the per-module CSV and text buttons retire with the results zip and everything is packed in the report export page — `data/` is the single tabular channel and no per-module replacement download is built | V | M / M | after #241 and #242, so `data/` is born corrected; **before #270** |
| 10 | **The summary report's four cross-cutting sections merge into the oracle report, then `build_report` retires** — `app/views/export_report.py` is the **only production consumer** of `content.build_report` (every other caller is a test), so note 57 D-57.6 retires a 2,632-line document it never names — and with it the only statement of the axis system and sign conventions either front-end makes (`conventions_tex.py`, 276 lines, sole consumer, three static TikZ diagrams) and the only FAR 23 Subpart C coverage matrix (`coverage.py`, 241 lines, sole consumer) *(design note 60 D-60.7…D-60.11, amending note 57 D-57.6)* (#278) | Axes and sign conventions, the FAR coverage matrix, the governing safety-factor table and the bundle manifest carried into the oracle report as a **front-matter group after the Introduction** (`oracle_content.FRONT_SECTIONS`, so the section plan stays derived and G-OR-2 survives re-cut on the OR-16 pattern); every other `content.SECTIONS` key declared superseded with its successor or its reason, in an audit table a guard reads (note 60 §5, gates 11/12) | V | M–L / M | after #245; **before #270**, which deletes `build_report` |
