...
| 4 | **Plots port as render-only helpers, written fresh** — V-n chart, three-view, span shear/BM/torsion, VMT envelope overlays with the external-CSV comparison; the `app/` implementations are the spec, not the source (CC F(72)/F(63) are M4-11b's own exhibit) *(note 57 D-57.4; lawful under C210-15)* (#267) | One plotting module in the shell reading the same result slices the pages render as tables; trim & stability sweep deferred (note 57 §8) | V | M / M | after #266 |
