...
| 49 | **Mean takeoff torque is factored — `is_load_unit` tests the unit alone** — an engine characteristic in load units reads as a structural load; the residue after OR-116 is a wrong SF cell *(note 48 §1.2/§2.4)* (#170) | A producer-set `quantity` excluding non-load quantities from factoring, one edit to `is_load_unit`, plus the guard test; `engine.py` is open now | V | S / S | — (the SF cluster: one session with #177/#179/#180/#15) |
...
| 52 | **One surface, two names: retire `fin_*` for `vtail_*`** — owner ruled 2026-09-06 (`CONVENTIONS.md` §7.2); mechanical rename plus a guard that no new `fin_` identifier appears (#223) | One name for the vertical tail across the call chain; lands **first** in the tail-geometry cluster, before #219/#220/#156 edit the files it renames | V | S / S | — (unblocked at the 0.8.2 cut) |
| 53 | **The plane a surface is defined in is positional, not declared** — `XYPoint`'s documented "butt line Y" is false for the vertical tail; five name-test branches decide *(owner's question 2026-09-07)* (#220) | One `surface_plane(component)` owner replacing the five branches; docstrings corrected; **no** user-selected plane field yet (it belongs with V-tail/cruciform support) | V | S / S | #223 first |
