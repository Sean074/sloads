...
design notes per step (live ones here — 21 (parked), 44, 49; notes 45–48
...
| 28 | **Note 49 amendment first: gate G-OR-49 contradicts OR-93** — "no `sloads/report/**` path multiplies" is unsatisfiable while OR-93 keeps the summary report ULTIMATE via `report/content.py`, whose multiplies the §5.1 change table omits; amend the note while it is AGREED-not-started *(review R-12)* (#182) | The gate re-scoped, or the `content.py` migration added to §5.1 with its cost | V | S / S | before the note-49 implementation starts |
...
| 32 | **Unstamped single-module runs bypass the governing SF table** — `oracle_app/results.py` and the comparison view call `registry.get(name)(project)` with no `stamp()`: a project override is silently ignored, non-load conditions keep the dataclass 1.5, #154's banner survives on that one surface *(review R-6)* (#177) | Stamping made structural at the registry entry point so an unstamped render is impossible | V | M / S | the OR-13 freeze lifting |
...
| 14 | Export deck-writing primitives out of `sbeam_bridge.py` (CH-4) — *moved from band D 2026-09-04: note 49's export sweep rewrites the same `_sf`/`_sf_str` helpers, so this rides that thread* (#15) | `_fmt`/`_sf_str`/`_stamped`/`_MAT1_*`/`_PBAR_*` in a shared module; the four private cross-imports gone | V | S / S | with note 49's sweep |
...
- **#170** — *Mean takeoff torque is factored: `is_load_unit` tests the unit
  alone.* ENGLOADS' 554.4 ft-lb mean takeoff torque reads as a structural load
  because the test is on the unit string, so the ULTIMATE channel scales it to
  831.6. Raised reviewing #154; design note 48 §1.2/§2.4 filed it rather than
  folding it in, because it asks what a *load* is, not which channel renders it.
  After OR-76 the exposure is case selection and the export path only — checking
  which of those it actually reaches is the first task. Shares an owner with
  `safety_factors.prescribes_factor`, whose load half is the same predicate.
  The producer `sloads/modules/engine.py` is **frozen** (OR-13).
