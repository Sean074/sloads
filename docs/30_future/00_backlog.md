...
- **Non-load conditions carry `safety_factor = 1.5`** — geometry, mass properties and design speeds claim a limit-to-ultimate factor that does not apply to them. No number is affected (the boundary scales by units and quantity) but the text report prints `[ULTIMATE, SF=1.5]` above a geometry table. **Not frozen code**: the source is the `ConditionResult.safety_factor` dataclass default plus `safety_factors.GoverningTable.stamp`. Owner's ruling 2026-08-30 — non-loads have no safety factor: `Optional[float] = None` rendered "N/A", fixed at the data model so every consumer inherits it, a mixed condition keeping its factor and showing N/A against its non-load rows. Tier L, ~12 consumer sites, drift guard owed both directions. **#154**; note 44 §10; raised in the 0.8.2 GUI review. **Design note 48 is AGREED (2026-09-04)** and this item implements inside it. The review found the factor applied far beyond case selection and the export; the rulings: module analysis becomes a LIMIT channel while the oracle GUI, the report and the export deck stay ULTIMATE through 0.8.2 (OR-76–OR-78); the factor is **stated, never applied** as the endpoint, with the last multiply removed in 0.8.3's own boundary note (OR-86); LIMIT becomes the global default with no per-artifact marker, retiring M4-15 in 0.8.3 (OR-81); and the factorless test gets one owner, `safety_factors.prescribes_factor` — *no load-unit value and no `case_ref`* — with `FAMILIES` untouched (OR-83). Measured: a stable 38 factorless conditions on both GA6 and Baron 58, `select`'s 6 critical wing cases protected by the `case_ref` clause. No schema hop (`ConditionResult` is not persisted) and no frozen file is edited. The Imperial baseline is regenerated deliberately and must move in `txt/*` and `csv/*` only (236 of 330 digests). Gates G-OR-44…G-OR-48.
