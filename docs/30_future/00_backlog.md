...
| 1 | **The export package reduces to one solver artifact** — four parallel model concepts (per-component decks, balanced deck, LRA beam, CONM2) share one id space, and the deliverable borrows its GIDs from artifacts that are not deliverables: `lra_model` imports `station_gid`/`tail_span_gid`/`tail_control_gid`/`sob_gid`, so GID 7 names one point in `wing_loads.bdf` and another in `lra_model.bdf`. ~2/3 of open export work is maintenance on the three concepts that are not the deliverable. `sbeam_bridge.py` is **not** mostly deletable — the oracle report reaches into it at seven sites, so the applied-load model is report infrastructure filed under the wrong name *(design note 56, owner rulings 2026-09-10)* (#263) | One LRA beam model with arbitrary properties proving the cases solve, plus a self-contained CONM2 mass model on its own CG grids; the applied-load model and report tables rehomed to `report/`; per-component decks deleted; `EXPORT_TARGETS` 10 → 2; id bands 25+ → ~8; 83 sbeam digest channels → ~11; two load-routing paths → one | V | L / L | design note 56 at AGREED. **Slices 1-6a, 7 and 6b-i shipped 2026-09-10/12**: the contract-statement owner, the report tables' move to `report/tables.py`, the per-component deck deletion (`EXPORT_TARGETS` 10 → **4**, not 2 — `gear` is a document under D-56.1 and `balanced` waits on §8's `roundtrip.py` question; sbeam digest channels 83 → **33**), and D-56.3's renumber (the LRA owns one contiguous run, `20001-30999`; gates 3 and 4 land; the registry's collapse to ~8 waits for D-56.9, since the bands left behind belong to the applied-load model). and D-56.4's mesh (schema 65 → **66**; the beam is cut loose from the load stations, counts settable per component at the ruled defaults, members run to their tips, `JOINT_MERGE_FRACTION` retires). and D-56.1's move (slice 6a: `sbeam_bridge.py` **ceases to exist** -- the applied-load family, the station numbering and the side-of-body loads move whole to `report/applied.py` with no shim; two guards pin one address per name; the band sweep and the CH-2 no-silent-defaults sweep both follow the code; `export/` 8,603 -> **5,769**). and D-56.6/D-56.7's CG grids (slice 7, taken **before** D-56.9 so the band it retires has no consumer left: one `GRID` per `CONM2` at the item's own CG in the new `mass-cg` band, unconnected by design, `inertia_only_cards` / `case_station_weights` / `flatten_mass_case` retired, gate 6 landing as GPWG rather than a solve — three roundtrip legs traded for one that reads the deck as shipped, the loss counted in the note; the mass model enters the digest baseline for the first time, 234 → 244 channels). and D-56.9's re-aggregation (slice 6b-i: rewritten by amendment 2026-09-12, rulings 13-15, after implementation found the original unbuildable — the set is **summed** onto the LRA grids, not relabelled, several aero stations and mass items onto one grid; `station_applied_loads` keeps the un-lumped set; gates 12 and 13 land; four `*_applied` digest channels re-stamp and 52 hold; the structural zeros go, because moving a force across an offset makes a couple, and the appendices say so). and D-56.10's comparison (slice 6b-ii: new owner `report/lumping.py` and a new **Appendix G** — the internal load at a cut is the outboard resultant transferred to the cut, one rule for V/M/T on all four members, evaluated twice about the same cuts, no solver; a table of the widest gap per channel over every case plus four deviation figures; D-56.10 amended twice before code — no shared critical case exists, the four registers are disjoint, and the gap is plotted rather than the two curves; measured, the fuselage is the outlier at 82–197 % of peak shear, filed as an argument for the spar carry-through becoming an owned mesh point rather than fixed in the slice that measures it). and D-56.8 + §8's unshipping (slice 8: the assembled deck stops being an artifact — four surfaces retire, `EXPORT_TARGETS` 4 → **3**, not 2, `gear` being a document under D-56.1; the report's manifest loses its row; `roundtrip.py` 529 → **186** as `wrap_as_stick_model` and its whole supporting cast retire with the last elementless deck, `_orientation` and `SPC_SID` moving to `deck_format` where the writers can reach them; three solver legs move onto the shipped beam deck and are stronger there, the gear leg stays on the un-aggregated set because the beam deck's trunnion also carries what D-56.9 summed onto it — Appendix G's finding arriving from the other side; #173/#176 close superseded). Remaining: **the tier-L §7 closure** (the issue dispositions and the standard-doc sweep). **The oracle-GUI buttons are #245's scope, not this row's** — ruling 6 says so; struck here 2026-09-11 |
