...
| **D-57.7** | **The seed button is built fresh in the surviving GUI with #78's hardening in its first version** — seeded rows loudly incomplete until positioned and tagged; merge/refuse/replace stated before the click. #78 re-scopes to this. **Landed 2026-09-13 (#269):** built fresh, and the answer is **merge** — the only one of the three that is idempotent. The behaviour, the matching and both sentences are owned in `sloads/` (`weight_estimate.seed_plan`, `mass_distribution.unplaced_warning`), which let the retiring `app/` page be pointed at the same owner rather than left holding a live destructive button for the rest of the milestone — not a port (nothing moved out of `app/`), and it means D-57.6 deletes a page that owns none of it. | *Port then harden.* Rejected: rule 4 — building the known defect into the new home to fix it later is the anti-pattern. |
...
* **#29, #148, #247–#252** close superseded per R-57.4; **#78** re-scoped
  (D-57.7) and **closes superseded at #269, landed 2026-09-13** — the seed
  now merges rather than replaces and an unplaced row says so, in both
  GUIs, from one calc-side owner; **#259** closes with the dashboard; **#255**'s fix site is deleted
