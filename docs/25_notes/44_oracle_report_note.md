...
`docs/10_standard/CONVENTIONS.md`, `docs/25_notes/32_oracle_gui_note.md`,
`docs/25_notes/34_oracle_user_guide_note.md`, `sloads/report/*.py`,
...
(OR-71 … OR-75) are in [design note 47](../25_notes/47_appendix_b2_chord_bending_note.md).
...
> **SUPERSEDED 2026-09-05 by [design note 50](../25_notes/50_fuselage_carry_through_note.md)**
...
| **OR-173** | **One case per engine, and no mirror asserted** *(decided in draft, owner reviewed)*. `failed_engine_index` selects one engine, which gives the fin **one** sense of load; SELECT's set carries both naturally (`−476.8` and `+1015.6` on `baron_58`). Each entered engine's failure is therefore run as its own case. The alternative — run one and mirror it — is correct only on a symmetric installation, which `baron_58` happens to be and a future asymmetric one will not; and a mirrored case is the report minting a case the analysis did not run, which is what OR-6 forbids. Section 10's OR-166 is the precedent, one section back: one row per engine, two mounts are two structures. `failed_engine_index` stays, as the selector for the single-case views that already read it. **Amended 2026-09-26 (#285, design note 66 §11; owner, in session):** the fin-load *sense* this decision was implemented with (`one_engine_out._vtail_sense`: `+y` engine → `−y` fin load) was reversed — every one-engine-out fin load (and its δ, α_tail) was published in the sense that *added* to its own engine's yaw while its β carried the nose's, contrary to SELECT's static convention (a nose-left `β > 0` carries a negative fin load). Invisible while each engine's case stood alone, since both senses were always delivered and the fin envelope was right; found when #285 put the fin beside the engine pair and the balanced case barely yawed. The fin now **resists**: `+y` engine → `+y` fin load, β the nose's (toward the failed engine). Magnitudes are unchanged; which engine's case carries which sign flips. The one-case-per-engine rule stands: ONENGOUT still runs every engine, and the balanced deck's reflected twin (D-66.13) is gated against the other engine's own run (G-66.10) and not used on an asymmetric installation. | **OR-6**, OR-166 |
