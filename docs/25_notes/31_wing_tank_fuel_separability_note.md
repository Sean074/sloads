...
2. **Assembled balanced case / LRA deck** (`balance/applied.py`, decision B-2): not
...
| `balance/applied.py` wing / body inertia and self-inertia; `mass_cards.py` header wing total | **parts** of the loading's rows (via `reacted_parts`) | fuel relief appears on the wing; CONM2 **cards stay one per row** (§8) |
...
| # | Scope | Tier |
|---|---|---|
| F1 | Field + io mapping + `SCHEMA_VERSION` 53 + validation (`wing_fraction_out_of_range`, `wing_fraction_on_wing_row`) + `reacted_parts` + `distribution()` on parts + the consumer sweep in `balance/applied.py` / `mass_cards.py` + the agreement drift guard (WF-3). | L |
