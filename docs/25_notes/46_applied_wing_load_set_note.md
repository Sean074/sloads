...
§Wing torsion physical sense, `docs/25_notes/44_oracle_report_note.md` §12
(OR-59…OR-64), `docs/25_notes/19_concentrated_wing_mass_nodal_split_plan.md`.
...
| **OR-70** | ~~**Appendix B.2 is not widened.**~~ **SUPERSEDED by OR-71 (design note 47, 2026-09-03).** The reason given here does not survive review: B.2 already prints `Mxx` beside B.1's `Mx` — the same pair of conventions — and 3.2's notation table already carries the sentence that separates them. B.2 now states `Mzz`. Kept rather than deleted, because the reasoning that failed is part of the record. | OR-69 |
