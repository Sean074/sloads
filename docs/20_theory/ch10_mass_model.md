...
| `Σ(WING parts of the case's loading) == 2 × (panel + Σ points)` per case | **One mass model (design note 63, #289, 2026-09-17):** the panel WINGINER integrates is half the `WING`-tagged `PANEL` items and its concentrated masses are the `POINT` rows of the case's own D-25 loading, so the itemized wing and the spanwise model are one reading, not two, and the identity holds by construction on every case of every fixture (`wing_state_tie`, G-63.1). It replaces note 29's tie `Σ(items tagged wing) == 2 × (panel_weight_lb + Σ concentrated)`, which compared two entries of the wing's mass — the entries the v67 migration retired. What is left to reconcile is the panel override (`panel_weight_override_lb`, reported by `wing_mass_tie` / `wing_panel_override_open` the way the fuselage table below is). The wing-tank share of a fuselage row can still be stated as `MassItem.wing_fraction` (note 29) and reaches the state through `reacted_parts` at the row's carriage; no shipped fixture needs it since its wing fuel became per-side tank rows |
