# Verification Baseline — Release 0.3.0

Permanent regression-baseline record for the `0.3.0` release (concept-loads v1;
`RELEASE_PROCESS.md` §4). Supersedes
[`01_verification_baseline_0.2.0.md`](01_verification_baseline_0.2.0.md). One
table per module: the condition checked, the printed Appendix A/B (or
worked-example) figure the manual gives, the reference-page citation, and the
tolerance the test enforces.
...
`0.3.0` test run: **501 passed, 0 failed** (`pytest`, coverage ~93%,
`ruff check sloads/ cli.py` clean, `scripts/smoke_test.sh` PASS,
`SCHEMA_VERSION = 33`). This document does not duplicate the
...
**LIMIT figures.** The calc — and therefore every oracle row here — is **LIMIT**
(the numbers the manual prints). The 0.3.0 ultimate-load chain (M4-7/M4-13 …
M4-16) scales deliverables by the per-case `safety_factor` at the render/export
boundary only; no oracle figure changed.

All fixtures are **Appendix A** (the 6-place GA single, `examples/ga6_normal.project.json`)
unless noted otherwise; the concept fixtures are
`examples/concept_heavy.project.json` (unswept) and the swept twin-turbofan
regional jet (D-1). **No Appendix B (10-place twin turboprop) printed table
...
## Oracle-vs-closure status (one page)

Where every module's verification stands at 0.3.0. **Oracle** = locked to a
printed Appendix A / worked-example figure (±0.1% unless a row states wider);
**closure** = validated by physics/algebraic closure because no legible printed
figure exists (per the backlog's instruction not to invent oracles).

| Module | Status | Basis |
|---|---|---|
| WTESTIMA | **Oracle** | Appendix A p133 (exact, INT-truncated prints) |
| WTONECG | **Oracle** | p136 |
| WTENV | **Oracle** | Ch 3 p21-22 (+1 approved deviation: aft ballast station) |
| WINGGEOM | **Oracle** | p141-142 |
| STRSPEED | **Oracle** | Appendix A speeds table + **p155 VD floor (M1-1)** |
| MACHLIM | **Oracle** | p160 |
| ENGLOADS | **Oracle** | p131 + inline turboprop hand-calc (2 approved 23.361 corrections); FAR 25 optional cases closure-locked |
| FLTLOADS | **Oracle** | p179-181 incl. the **landing-config set (M1-2, p179 polynomials → p181 case 89)** |
| SELECT | **Oracle** | Critical-loads summaries + Ch 9 case 202 (+1 approved deviation: **23.427 candidate set, M1-4**) |
| BALLOADS | **Oracle** | Ch 9 case 202 (+ SELECT cross-check) |
| AIRLOADS | **Oracle** | Ch 7 p46-47 / p161-162 |
| AIRLOAD4 (swept branch) | **Closure** | reduction invariant + **swept renormalization closure (M1-3)**; no printed Appendix B table |
| WINGINER | **Oracle** | p217-221 |
| NETLOADS | **Oracle** | p206/p222 |
| TAILDIST | **Oracle** | p237/p245 (flaps-extended chordwise rows deferred, L-2) |
| AILERON | **Oracle** | p200 |
| FLAPLOAD | **Oracle** | p201 |
| TABLOADS | **Oracle** | p202 |
| LGFACTOR | **Oracle** | p236 |
| LANDLOAD | **Oracle (geometry) / closure (wheel matrix)** | p230 oracle; p231-233 OCR-garbled → legible-cell spot-checks + algebraic closure |
| ONENGOUT | **Closure** | sub-formula exactness + transient physics closure; no printed twin table (D-5) |
| body_loads (modern) | **Closure** | ΣFz≈0 equilibrium; moment closure open (M4-1, caveat-stamped) |
| configuration (modern) | **Closure** | closed-form trapezoid identities + Appendix A plausibility band |
| Concept mode (AIRLOADS/NETLOADS/export) | **Closure** | recovered-CL, air+inertia=net, FORCE/MOMENT card re-summation |
| sbeam export bridge | **Closure** | card re-parse + force/moment re-summation = root totals × per-case SF |

Twin-turboprop (Appendix B) cases remain **closure-locked, not oracle-locked**
— Appendix B is not bundled (decision D-5; blocks L-9).
...
**VD floor, no chosen speeds (M1-1, new at 0.3.0).** Appendix A p155, Cat N:
FAR 23.335(b) requires `VD ≥ max(K_d·VCmin, 1.25·VC)`; here `1.40 × 141.8 =
198.53 kt` governs (STRSPEED.BAS `V2DMIN = K2*V1CMIN`, lines 380/390). The
pre-M1-1 code returned 177.26 kt — 10.7% non-conservative.

| Condition | Printed figure | Tolerance |
|---|---|---|
| Dive speed VD (no chosen speeds) | 198.53 kt | 1e-3 |
| Minimum dive VD(min) | 198.53 kt | 1e-3 |
...
**Flaps-extended (LANDING) configuration (M1-2, new at 0.3.0).** The
landing-config aero polynomials are printed in the Appendix A p179 input
listing (the 0.2.0 baseline's "not in the repo" claim was wrong — they sit
alongside the cruise set) and are transcribed into the `flight_envelope` test
fixture. The corner set (FLTLOADS.BAS subr 3000) closes at VF/n≤2, and the
`BAL 1.4VSF` balancing point — which balances at 1.4× the **1-g** flaps-down
stall (`STALL 1GL`, FLTLOADS.BAS p300-302; the pre-M1-2 code used the 2-g
stall, ~2.2× too large a tail load, review T2) — is oracle-matched:

| Condition | Printed figure (p181, LANDING CG5 case 89 "BAL 1.4VS") | Tolerance |
|---|---|---|
| Balance speed V | 83.6 kt | 5e-3 |
| Balancing tail load LT | −430 lb | 1.5e-2 (small CG-moment residual) |
| Balance α | −2.54° | abs 0.12 |
| MAN 2G VF: NZ / V | 2.0 / 105.5 (VF) | abs 0.01 / 5e-3 |
| STAL 2/3G: NZ | 2/3 | abs 0.01 |

The fuller flaps-extended chordwise rows (SELECT→TAILDIST, CG5–7, printed
cases 81/106/88/108) remain deferred to L-2.
...
| UNSYMMETRICAL total | **-1204.7** (manual sample prints -1111.8) | 5e-3 |
| UNSYMMETRICAL RH side load | **-700.4** (was -646.4) | 5e-3 |
| UNSYMMETRICAL LH side load | **-504.3** (was -465.4) | 5e-3 |
| UNSYMMETRICAL other-side % | 72.0 | abs 0.1 |

**Approved oracle deviation — 23.427 candidate set (M1-4, approved 2026-07-20;
supersedes the 0.2.0 rows).** SELECT.BAS lines 6070-6175 include the unchecked
maneuvers in the unsymmetrical candidate array (`L(5)=U1CK, L(6)=U2CK`) and take
the max over all 12; 23.427(a) spans "the loads prescribed in 23.421 through
23.425", including the 23.423 unchecked case. For the GA6 the DN unchecked
maneuver (U2CK = -1397.835, case 274) governs over the down gust (-1292.8), so
the unsymmetrical total is **-1204.7** (RH -700.4, LH -504.3, 72%). The
Appendix A *sample output*'s -1111.8 came from a superseded SELECT.BAS that
excluded the unchecked cases — inconsistent with its own Appendix C listing; the
listing + the CFR are authoritative. Register:
...
LT50) only. The landing-config polynomials themselves are transcribed and
oracle-matched at the envelope level (M1-2, see FLTLOADS above); the chordwise
...
- **Redistribution-direction closure** — with 25° sweep, root `ccl_additive`
  ...
  target).
- **Swept renormalization closure (M1-3, new at 0.3.0)** — on the D-1 regional-jet
  fixture (Λ=24°, twisted), the swept span load re-integrates to the operating
  CL (`recovered_cl == aero.target_cl`, `rel_tol=2e-3`); pre-M1-3 the COL20
  renormalization was missing and the wing lost ~9.6% of its lift
  (recovered 0.452 vs target 0.50).
- **Listing-traceable sweep subroutine (M1-3)** — `_sweep_operating` reproduces
  an independent reconstruction of AIRLOAD4.BAS's
  `COL18 = (1−2·YE/B)·2·(1−cos Λ)` → `COL19 = COL16−COL18` → `COL20 =
  COL19/CL(COL19)` strip-by-strip (`rel_tol=1e-12`), and the renormalized load
  re-integrates to the operating CL exactly.
