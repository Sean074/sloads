...
- **The case's loading is the mass state of every inertia load (design note 63,
  ...
  tag: the taper cannot tell an engine from a spar by inspection). The half-span
  models run the starboard `POINT` set and mirror it, so a searched loading is
  laterally symmetric by construction and an asymmetric entered one is named
  (`wing_mass_asymmetric`). The per-case identity `Σ WING parts of the loading ==
