...
| **The cross-cutting sections a report prints** — axes and sign conventions, the governing safety-factor table, the FAR 23 Subpart C coverage matrix and the bundle manifest. Both documents printed them while both existed; the summary report retires with `app/views/` at #270 and the oracle report carries them as front matter, so they have **one builder** and the merge cannot become a copy that drifts. What the retiring document leaves is declared key by key, in a table a test reads (note 60 D-60.7…D-60.11, #278) | `sloads/report/front_sections.py` (`conventions_section`, `governing_factors_section`, `coverage_section`, `package_files_section`, `SUMMARY_DISPOSITION`); the slot is `oracle_content.FRONT_SECTIONS`, and the heading is an argument so no builder writes a section number | `tests/test_front_sections.py::test_the_two_documents_print_one_builders_output` + `::test_every_merged_section_is_built_and_carries_content` + `::test_every_retiring_section_is_declared_merged_or_superseded` + `::test_the_merged_owners_keep_a_consumer_that_outlives_the_summary_report` |
...
| **Which airplanes a design is compared against, and where each of its own numbers comes from** (the bundled reference fleet is nominal published specifications — Imperial, never converted and never a FAR input — and the **subject**'s metrics each have a stated priority chain: MTOW from `cg_cases.max_takeoff_weight` then WTESTIMA, empty weight from the item database then WTESTIMA, area/AR/span from the parametric layout then the WINGGEOM wing surface. The chain is the part of this page with a defect history — the item-database total sat at the head of it until 2026-08-15 and plotted a regional jet 1,800 lb heavy — so it has one owner and the front-ends ask it. Note 57 D-57.5's *rewritten, not imported* is superseded here by note 60 D-60.1's reasoning: a second derivation is a second owner) | `sloads/fleet.py` (`REFERENCE_CSV`, `reference_fleet`, `subject_from_project`, `fleet_stats`) with `sloads/report/fleet_figures.py` for the six scatters and `app_shell/fleet_view.py` for the one rendering both pages use | `tests/test_fleet_figures.py::test_the_reference_csv_has_exactly_one_reader` + `::test_neither_page_derives_the_subject_or_the_figures` (both front-ends) + `::test_every_bundled_example_places_against_the_fleet`; `tests/test_reference_aircraft.py` guards the data's shape |
