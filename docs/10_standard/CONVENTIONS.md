...
| **Whether a mass reconciliation reaches the reader** (the suite carries two mass models — the itemized `weight.items` database and the entered `fuselage_mass.stations` / `tail_mass.panel_weight_lb` tables — and every check that compares them is stated in the **issued document**, not only on the Weight & Mass screen. Before #257 three of the four were screen-only or had no consumer at all, while the entered station table sits 13–41 % under the beam on every shipped fixture. A new check is stated or carries a written exemption; it is never silently unread) | `sloads/mass_distribution.py` (`partition_closes`, `wing_mass_tie`, `fuselage_reconciliation`, `tail_reconciliation`, `untagged_tail_surfaces`) — read by `report/oracle_sections.py` (`_beam_reconciliation`, `_untagged_surface_statement`, `_tail_mass_provenance`, `_wing_mass_tie_sentence`) | `tests/test_mass_distribution.py::test_every_mass_reconciliation_is_read_by_the_issued_document` (by name, `ast`-parsed) + `::test_the_unstated_check_exemptions_are_not_stale` + `::test_the_issued_document_states_every_mass_gap_it_ships_with` (by effect, on the rendered document) + `tests/test_oracle_report_fuselage.py::test_the_beam_states_how_far_the_entered_table_is_from_it` + `::test_a_surface_no_weight_item_claims_is_stated_on_the_beam_that_carries_it` |
