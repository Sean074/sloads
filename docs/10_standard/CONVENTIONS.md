...
| **An engine's thrust line, and the airplane-axis resolution of the torque and thrust that act about it** (the axis is the unit vector from the engine CG to the propeller hub — forward by construction — derived from the two stations the project already enters and never entered a third time; where they coincide it falls back to airplane **−x** and every deliverable marks it ASSUMED. "Clockwise from the pilot's view is positive" **is** the right-hand sense about that line, because the pilot looks along it, so the module's scalar is rotated and never flipped. Written here rather than at a call site because it is an axis resolution, which §1 already makes this module's — note 44 §20 OR-161/OR-162) | `sloads/export/coordinates.py` (`engine_thrust_axis`, `engine_applied_load`, `ASSUMED_THRUST_AXIS`) | `tests/test_oracle_report_engine.py::test_the_printed_components_are_the_resolution_of_the_printed_scalars` (asserted **through** the owner, so a component added later cannot inherit a literal) + `::test_a_derived_axis_is_the_direction_from_the_mount_node_to_the_hub` + `::test_an_assumed_thrust_axis_is_marked_and_a_derived_one_is_not` |
