...
(**OR-117**, `deck_format.basis_sentence`), gated by **G-OR-73**; every rendered
...
- **Writes:** `Project.weight.cg_cases` — named `CgCase` rows (weight, xcg, zcg, the `analyses` the case is run for, a landing `role` where it is one of LANDLOAD's three, and — since decision **D-25**, schema v50 — an optional `loading`) entered once. **The loading is the mass state of every inertia load (design note 63 D-63.1, #289):** WINGINER, NETLOADS, `body_loads` and the balanced deck read the case's loading (entered, else the `derive_case_loadings` search) through `mass_distribution.wing_mass_state`, so there is one mass model per case and the `wing_mass` slice holds no mass of its own. `weight.max_zero_fuel_weight_lb` (MZFW, D-63.5, #292) is the third design weight beside MTOW and MLW (`cg_cases.max_zero_fuel_weight`, `0` = not entered, the estimate `max_zero_fuel_weight_estimate` = OEW + max payload offered and never written) and, when entered, `seed_flight_cases` appends three FLIGHT cases **with their loadings** (`cg_cases.MZFW_CASE_NAMES`: `mzfw aft`, `mzfw fwd`, `full fuel aft`) found by `mass_distribution.seed_loading_search` — the heaviest zero-fuel loading toward each envelope edge under MZFW and the full-fuel loading trimmed to MTOW at the aft edge, a whole-row loading where one is inside the limits and otherwise one payload row **clipped** onto the limit; a seed that coincides with a case already seeded (D-25a's weight band, the search's CG tolerance) is not written. **The loading is entered on the case (design note 63 D-63.9, #290):** the Payload Cases group of the Weight & Mass Properties page renders each case's `loading` inside the case's own row — the discretionary rows aboard as a multiselect over the `DISCRETIONARY` items, a fraction widget for every row that may be partial (a consumable row aboard, a named discretionary row; `1` is whole and is not written), an optional ballast row with its own station and waterline, and the D-25a echo beside them (the loading's own weight and CG against the case's, in the display units, from `entered_loading` and the two tolerance owners). A case with no loading offers **Add loading**, which enters the searched loading *as found* (`mass_distribution.loading_definition_of`, so no load moves and every later edit is the user's); **Remove loading** takes it off again — both named clicks, so visiting the page attaches nothing (`tests/test_gui_journey.py`). The same page carries the read-only **Mass cases** table (`mass_distribution.mass_case_summary`: id, name, role, analyses, the entered W/Xcg/%MAC/Zcg, the loading's own W/Xcg/Zcg and whether the echo agrees, fuel aboard by the item `consumable` flag, payload, ballast lb and %, the wing panel and points per side, and the source — entered / searched / database — with its reason), and the Wing Loads page shows the per-case WING parts read-only (`wing_parts_summary`). Concept-mode validation warns `case_loading_missing` on a FLIGHT case whose loading the search derives but nobody entered (D-63.1; the two concept fixtures enter theirs).
- **Notes:** exists to stop the CG-envelope chart (WTENV) and the flight-envelope balance (FLTLOADS) from carrying two independently-edited copies of the same loading scenarios (the Phase-D GUI assessment's finding #2, "no enforced single source of truth for shared inputs"). Decision **G-3** finished the job: `FlightLoadsInput.cg_cases` (a derived copy kept in step by a *page* rather than by the model) and `LandingInput.cg_cases` (the one list that never joined the SSOT) are **removed**, each case states the `analyses` it is run for, and every consumer reads `sloads.cg_cases` rather than filtering for itself. Pre-v19 files carried the scenarios only under `flight_loads.cg_cases`, and the v46 hop tagged those `FLIGHT` and folded `landing.cg_cases` in as `GROUND` with roles from the canonical names; both hops retired at #93 with the rest of the migration chain, after the shipped fixtures were re-stamped through them (verified output-neutral, flight and ground sets identical on all six). **Decision D-25 (2026-08-15, schema v50) gave the case an explicit loading definition.** A `CgCase` states a weight and a CG — the corner of the weight/CG envelope, which is the real engineering input on a reference airplane — but not *what loading produces it*, so the mass model behind each case was **searched for**: `mass_distribution.derive_case_loadings` tries the `2^n` discretionary subsets of `weight.items` plus a ballast row solved from the residual, accepted only under the 10 % credibility gate, which reaches 7 of 18 shipped cases. `CgCase.loading` (`LoadingDefinition`) states it instead: `aboard` (the discretionary rows carried — `EMPTY`/`MINIMUM` rows are aboard by definition and naming one is rejected), `fractions` (a `consumable` row — or, since #292, any discretionary row: a part-filled hold — scaled to a partial value in `(0, 1]`; station and inertias unchanged, so the tank layout is preserved as G-5's burn-down preserves it) and an optional `ballast` row carrying its own waterline. Four rules go with it: the entered loading is **authoritative** and the case's `weight_lb`/`xcg`/`zcg` are a **checked echo** of it within `max(0.5 lb, 0.1 %)` and `0.5 in` — reported by `case_loading_checks` and as the `cg_case_loading_echo` / `cg_case_loading_invalid` findings, never absorbed by bending the loading (**D-25a**). A *searched* loading that carries no ballast has nothing to move it either, so the search accepts one only within the same `0.5 in` on station **and** waterline, and `case_loading_checks` holds it to that band rather than to machine precision, on every case, ground as well as flight; the check is stated under the oracle report's 2.2 case table, naming any case whose loading is not the case (**#300** — the search once tested the station alone, and `baron_58`'s `aft gross` flew a loading 4.12 in below the waterline every module read, `concept_regional_jet`'s `fwd max landing` one 2.16 in above it). `cg_cases.seed_flight_cases` alone searches with `match_waterline=False`, since its waterline is the search's answer (D-26a). The remaining rules: the weight database stays the mass SSOT (**D-25b**); the slice is **optional**, with the subset search as the fallback, so every pre-v50 file behaves bit-for-bit as it did (**D-25c**); and the ballast-credibility gate applies to a *solved* ballast only — an entered one is an engineering statement and exports with its fraction stated (**D-25d**). `CaseLoading.entered` carries the provenance into the report's payload-case table, the page and `mass_case_rows`. Design note: [`../25_notes/25_d25_cgcase_loading_note.md`](../25_notes/25_d25_cgcase_loading_note.md). **Decision D-26 (2026-08-15) amended D-25's "the corner points stand" for the fixtures**: where a case is not a loading its own weight database can produce — measured at 15–60 in in station and 4–31 in in waterline on five of six shipped fixtures, and unreachable *by construction* where the heaviest case weighs exactly the all-up weight — the **case data is corrected to the database**, not bridged with ballast. Three rules follow: `zcg` is a derived echo of the loading rather than an independent corner point, since WTENV's envelope is weight against *station* and the waterline never had one behind it (**D-26a**); lumped payload rows are **zoned** (fwd/aft cabin and hold) preserving each fixture's discretionary `Σw`/`Σwx`/`Σwz`, because a database of two or three point masses admits exactly one loading at gross weight and the fwd/aft corner pair collapses (**D-26b**); and a database whose rows sum to exactly MTOW carries no usable capacity, so no two loadings weigh the same (**D-26c**). One consequence in the calc: a `LoadingDefinition` states which items are aboard, **not what the airplane weighs**, so `balance._ground_target` drops it when 23.473(a) re-weights a ground case to MTOW and falls back to the subset search — the one route that solves for weight as well as station. Every case of every shipped fixture now states a loading, and none of them carries ballast. Design note: [`../25_notes/26_pri5_fixture_loadings_note.md`](../25_notes/26_pri5_fixture_loadings_note.md). **Decision D-27 (2026-08-17) turned the fixture question round: the load analysis evaluates the weight/CG limits the user defines.** A FLIGHT case is a WTENV structural-limit point *by construction* — `cg_cases.seed_flight_cases` writes the four FLTLOADS.BAS prompts (aft gross, fwd gross, fwd regardless, minimum weight) plus one mid-envelope gross-weight case (`mid gross`), `weight_lb`/`xcg` the limit's own and `zcg` the closing loading's waterline (D-26a kept); the loading is **derived** (`loading is None` → the subset search with solved ballast under D-25d's 10 % gate), so "the case is on the limit" and "the database can fly there" stay two statements checked against each other. On the Appendix A airplane the seed reproduces CG1..CG4 (85.1 / 77.49 / 72.64 / 73.09) — it *is* Ch 3 p21's definition. The four type fixtures were reconciled to it: their fuselage-carried item stations were on a datum ~20–65 in aft of the wing polyline's (hand-entered, 2026-07-15), so every D-26 case sat 15–25 in aft of the entered `%MAC` aft limit; the stations were shifted onto the wing datum (empty CG at a type-plausible 18–35 % MAC; wing- and tail-tied rows untouched; the RJ's cabin/holds re-spaced), and every fixture case now closes with 0–8.5 % ballast and sits inside its envelope. Guards: `tests/test_cg_cases.py` (the fixture cases *are* the seed; every case inside its envelope; the ga6 reproduction) and the new `mass_item_outside_body` warning (`validation._check_mass_items_in_body`: a `fuselage`-carried item ahead of the outline's nose or behind its tail — the three-view's claim made structural; fore/aft extent only). Only `concept_heavy` still enters a loading (one case, one loading). Register: D-27 in [`../25_notes/03_resolved_decisions.md`](../25_notes/03_resolved_decisions.md).
...
- **Notes:** Many downstream modules read `geometry.surfaces`. Model surfaces as a list keyed by surface name so one calc serves all. **Step G1:** WINGGEOM (the `wing_geometry` module) no longer has its own page — it is folded onto the single **Geometry** page (`configuration_layout.py`, `FOLDED_MODULES`), whose "Lifting-surface planforms" section is the surface polyline editor. Has graphics: the top-view planform outline per surface (`sloads.modules.wing_geometry.surface_top_outline`, a presentation-only helper — no new `ConditionResult`), reused by the Geometry page's three-view for the wing outline. **Boundary-line resolution (note 54 D-54.1, v65):** the integrator runs on `tail_geometry.resolved_surfaces` — a control surface (elevator/rudder/aileron/flap, `tail_geometry.CONTROL_PARENT`) entered without its own trailing edge derives it from the parent's over the control's span (the two are one physical line, #25), and an entered tail-group control TE off the parent's is refused; every other consumer of a control polyline (AIRLOADS' Schrenk pass, the report's planform figures) reads through the same resolver.
...
- **Writes:** the full balanced V-n matrix (one `VnPoint` per condition × CG × altitude: V, NZ, α, G, CL, M(W+F), LZW, **LT**, DX) and the balancing tail load per point → **`Project.envelope`** (`EnvelopeResult.vn` + `.tail_balance`), consumed by SELECT. The pure entry point is `flight_envelope.build_envelope(project) → EnvelopeResult`; `run(project)` returns the per-point `ModuleResult`. **`VnPoint.case_refs` is a list (schema v64, note 44 OR-200):** SELECT stamps each point with the `CaseRef` of *every* critical condition it was selected for, appended in emission order (wing, htail, vtail, fuselage). It was one slot until 2026-09-07 and a point is routinely selected more than once — on `ga6_normal` V-n case 14 is VT-01, VT-02 **and** VT-03, case 74 is HT-03 and HT-09, case 110 is W-03 and F-01 (case 30 until #164 put the fixture at three altitudes) — so the single slot kept only the last write. Nothing read the field outside serialisation, which is why it is a latent defect fixed at its first reader rather than a shipped one. `_stamp_case_refs` clears before appending, so stamping one envelope twice is the same as stamping it once.
- **Published (note 44 §23, OR-194 … OR-201):** the matrix is the oracle report's **Appendix A**, one row per balanced point, over two column groups — the flight state (CG, config, altitude, case, condition, V, NZ, α, G, CL) and the balancing loads with the case ids each point was selected as (M(W+F), LZW, LT, DX, NX, W/F/HT/VT). The split is by column group and never by row: nineteen columns do not fit A4 landscape at any size the renderer sets (measured 662.7pt against 424.9pt), and the file beside it, `<project>_vn_conditions.csv`, carries all nineteen in one flat row. It reproduces Ref 1 Appendix A p179-185 with three additions the owner asked for — the CG id, the altitude and the configuration as *columns* rather than as block headings — and one the analysis needs: **NX**, the inertia drag factor `−DX/W` the drag leaves the balance as (`aero_curves.inertia_drag_factor`, the one owner `select` and `wing_inertia` both read). Thrust is **not modelled**: the balance solves the normal force and the pitching moment and writes no longitudinal force equation, so every condition is thrust-off and the airplane is in longitudinal equilibrium by d'Alembert. The appendix states this; modelling power is filed, not done.
- **Validation:** Appendix A "V-n Data" p179-180 — the cruise balanced matrix per CG case. The AoA balance converges NZ only to ±0.005 (FLTLOADS.BAS line 4130), so low-load-factor quantities carry ~0.5% noise; LT and the corner speeds/load factors match tightly. **The shipped `ga6_normal` fixture balances at Appendix A's own altitude set, 0 / 12,000 / 18,000 ft (#164, 2026-09-20)** — the manual picks five of its six critical wing conditions at 12,000 ft, and until #164 the fixture carried sea level alone, so every delivered case stated `0 ft` while the loads reproduced; the SELECT oracle (`tests/test_select.py`) reads the fixture as shipped and guards the list, so the oracle can never again pass on an altitude set the fixture does not deliver.
- **Convergence (#33, 2026-08-22):** each balance states how it ended. The angle-of-attack iteration has one acceptable end and **refuses** otherwise (`SolverFailure`, `sloads/convergence.py`) — before this it returned the angle it gave up at, so a CG the airplane cannot trim at produced a point labelled 1 g that was not (measured: NZ 0.658, on into SELECT and the decks). The dynamic-pressure iteration has a second acceptable end, **clamped**: the Mach cap pins the true airspeed, `q` reaches a fixed point off the stall line and no further trip can move it. That is stall-limited flight, which **23.333(b)** excludes from the manoeuvring envelope (decision **D-30**), so the point is reported with its state rather than refused; `EnvelopeResult.clamped_cases`/`is_clamped` carry it, derived and never persisted. On the shipped fixtures the clamped set is **empty** since #260 (2026-09-21) — `atr42_100` reached it at nine, then twelve, Mach-capped corners at 25,000 ft while its wing was 18 % small, and the reconciled fixture no longer does; `tests/test_convergence.py` exercises the state on that airplane balanced at 35,000 ft, and the Aerodynamic Data page's stall-clamp margin flags the same rows — pinned to agree, so #32's marker reads one owner. Bit-identical to the pre-#33 spin-out (all pins and the frozen digest unmoved) and 4.3× faster on that fixture.
...
(PHAA/PLAA/PMAA/NMAA + accelerated-roll + steady-roll TORS, and — above the
`.BAS`, design note 62 — the negative triad's NHAA/NLAA and the
load-factor-extreme pair PNZ/NNZ, ten slots in all), (2) the
...
- **Writes:** the governing (critical) flight-load set per surface → `Project.envelope.critical`. Per Ch 9 this is **much more than selection** — SELECT *computes* the rational critical loads: wing loads (PHAA/PMAA/PLAA/NMAA, accelerated & steady roll; NHAA/NLAA/PNZ/NNZ above the `.BAS`), rational + balancing + maneuvering + up/down-gust + unsymmetrical **horizontal** tail loads, **vertical** tail loads (23.441/23.443), and **fuselage** loads (23.301/23.331/23.351/23.471; Ch 9 + Ch 15 net fuselage).
- **Validation:** Appendix A/B — the selected critical points (`SELWGLDS/SELHTLDS/SELVTLDS/SELFSLDS`).
- **The wing search above `SELECT.BAS` (design note 62, #288, 2026-09-17).** The `.BAS` searches three positive angle-of-attack slots and **one** negative; the negative side now carries the same triad — **NHAA** (largest resultant among STALL −N / STALL −1G), **NMAA** narrowed to the VC pair (MAN −C / GUST −C; its Appendix A pick is unchanged), **NLAA** (MAN −D / GUST −D) — and a load-factor-extreme pair, **PNZ** (largest `nz` over every positive-family label, tie-break largest resultant) and **NNZ** (most negative `nz` over every negative-family label); two `nz` within the balance's own band (`select.NZ_TIE_BAND`, twice `constants.NZ_BALANCE_TOL`, the tolerance FLTLOADS iterates to) are one load factor and the resultant decides (#294). A negative slot admits a candidate only when the **wing's** lift is negative (`LZW < 0`) and PNZ only when it is positive — the sign tested is the wing's, not the airplane's load factor (D-62.2) — and a slot with no eligible point is **empty**, a gap in the W- band. A PNZ/NNZ point that is already another **delivered** slot's point is not delivered twice (D-62.8's coincidence rule, `case_ids` M4-2 decision 1) — a delivery rule, applied once in `select_wing` after the note 63 re-pointing, never to the air picks (#294: run on the air picks it emptied NNZ against NMAA's pick, NMAA then moved to a zero-fuel case, and the 23.337(b) extreme left the Baron's and the RJ's deck); `select.air_picks` therefore lists every slot the matrix can fill, and the loss side is gated — every eligible PNZ/NNZ extreme is carried by some delivered wing case (G-62.1). All four join `SYMMETRIC_WING_CONDITIONS`, so the LRA deck gains up to four symmetric subcases closing under G-OR-72; a slot whose CG case has no derivable loading is a `loading-not-derivable` skip like the six before it (`baron_58` W-07/W-08/W-09 at "fwd gross"/"fwd regardless", until #290). The six Appendix A picks reproduce unchanged (gate 1); the new slots are closure-gated on every fixture's frozen pick (`tests/test_select.py` G-62.1/G-62.2, `test_balance.py` G-62.3, `test_case_ids.py` G-62.4). Fixtures' hand-entered `wing_mass.cases` are untouched, so no new slot reaches WINGINER/NETLOADS until note 63 (#289).
...
- **One quantity, one key (#222, 2026-09-20).** The fuselage set publishes the wing reaction `LZW − NZ·WW` as `fuselage_load_on_wing` and the tail load as `tail_load` on every block that states them, the sign carrying the sense (M4-9: the key is the machine identity, the label display text). Until #222 the two down blocks keyed the reaction `fuselage_down_load_on_wing` and the GREATEST NZ block keyed the tail load `balancing_tail_load`, so the report's §4.3 table carried two keys per column to fold them (note 44 OR-14, filed under the 0.8.2 freeze). The guard in `tests/test_select.py` sweeps every component of every shipped fixture for a label under two keys or a key under two labels, and pins the fuselage key vocabulary. No value moves; three label cells in the SELECT and BODY_LOADS CSV/text outputs do.
...
- **Notes:** Central junction. Reads V-n data from FLTLOADS + geometry (WINGGEOM) + inertia (WTONECG). Per UG Table 2.2 it feeds **AIRLOADS, AIRLOAD4 (iterative — see AIRLOADS), WINGINER, TAILDIST**. NETLOADS/component modules consume `critical` indirectly via those. **Step D5:** `CriticalLoadSet.selected_case_ids` is an **opt-out GUI selection** — the Critical Loads page persists which computed conditions the engineer keeps for the deliverable (empty = no filter, every condition kept, the default and the whole behavior for older projects); `CriticalLoadSet.selected()` applies it. Only the **Results Review** page's display reads `.selected()` — every structural calc module (WINGINER/NETLOADS, `body_loads`, the sbeam export bridge) deliberately keeps reading `.conditions` unfiltered, so the selection can never silently drop load cases from a deliverable's structural sizing, only from the GUI summary. (D8.3 is expected to wire the export bundle to this same selection — not yet done.) **M2-4:** the governing-loads tables on **both** the **Results Review** headline and the Flight Envelope **Critical Loads** tab render through one shared `report.governing_loads_table(conditions, system, sf)` — load columns are LIMIT (plain units + the `SF` column stating the factor that was not applied), dimensionless/speed columns (n, CL, V) likewise unscaled and unmarked, absent cells `"—"`. **Review F-R1 (M4-8 Layer-1 report-side slice):** the factor is **per case** — each row scales by its own `CriticalCondition.safety_factor` (14 CFR 23.303 → 1.5 by default, 1.0 for a case already at ultimate) and its `SF` cell states that row's factor, matching the export side (`deck_format.case_sf`); the helper takes no caller-supplied `sf` override, so a report figure and its bulk-data card cannot state different factors for one case. **M4-8 / G-11 (2026-08-14):** that per-case factor is now itself a derived view — `sloads/safety_factors.py`'s governing table classifies each case by its FAR reference and writes the carrier at `registry.run_all_modules`, `report.content.component_loads` and `balanced_run`, so a project-level override reaches the report column and the bulk-data cards together or not at all.
- **The boundary-line model derives the seam (note 54 D-54.1 / #25, schema v65, 2026-09-10):** the planform-geometry scalars of `TailLoadsInput` (9 of 17) and `VTailLoadsInput` (10 of 15) — every area, span, MAC, MAC station and aspect ratio, marked `[D]` in the dataclasses and named in `models.inputs.HTAIL_BOUNDARY_DERIVED` / `VTAIL_BOUNDARY_DERIVED` (`{field: deriving surface}`, partition-guarded in `tests/test_empennage.py`) — **blank-derive from the entered boundary lines** through `tail_geometry.boundary_derived_scalars`, consumed by `effective_tail_inputs`/`effective_vtail_inputs` and by `resolve_tail_planform`'s scalar read (note 36 OV-1: typed overrides, blank derives). The lines are the tail group's five (D-54.1): tail LE, tail TE and control LE as the `geometry.surfaces` polylines; the control's **hinge line** (`SurfaceInput.hinge_line`, v65 — a blank SEFWDHL/SEAFTHL or SRFWDHL/SRAFTHL pair derives from its area split, `control_hinge_areas`); and the control's **TE is the parent's** — entered empty it derives (`derived_control_trailing_edge`, byte-identical to Appendix A's printed elevator/rudder tables on `ga6_normal`), entered typed it is held on the parent's line (`validate_control_trailing_edge`, hard on the tail groups; the wing controls are exempt from it, and since #216 reconciled the two estimated aileron polylines every shipped wing control passes it — the furthest is 0.04 in off, against the 9.4 in of 2026-09-10 — so extending the refusal to them is a one-line change awaiting the #260/D-54.6 wave that owns their geometry). A **typed** scalar stays authoritative — every Appendix A pin is untouched and the Imperial digests did not move — and the 1 % `validate_tail_planform` guard compares **entered** scalars only, retiring field-by-field exactly as they stop being typed. Appendix A's printed horizontal-tail/elevator/rudder figures are thereby **predictions** of the model (±0.1 %, `tests/test_tail_geometry.py::test_the_boundary_model_predicts_the_printed_appendix_a_figures`). The aero/control-setting/mass fields stay entered permanently; `LayoutInput.htail_dihedral_deg` (D-54.8) is **declared only** — no load reads it (guarded) until note 51's method spends it.
...
- **Reads:** the **`Project.wing_mass`** input slice (`WingMassInput`): the panel *shape* — tip/root area-density ratio, inboard rib butt line, wing-reference-plane waterline + dihedral (**`wrp_waterline`/`dihedral_deg` are derived from the parametric wing on `Project.geometry`, Step M2-6 — not stored, GUI read-only**) — and the critical `WingLoadCase` list. **The mass is the item database, read per case (design note 63, #289, v67):** the panel weight is half the `WING`-tagged `PANEL` items (`mass_distribution.panel_weight`; `panel_weight_override_lb` is the OV-1 override, blank derives) and the concentrated masses are the `POINT`-carriage `WING` rows of **the case's own mass state** — the D-25 loading of the FLIGHT case `WingLoadCase.cg` names, else the referenced V-n point's CG case, else SELECT's condition of the same label, else the whole database stated in-band (`wing_inertia.resolve_mass_case`, `mass_distribution.wing_mass_state`). The per-case `Nz`/`Nx` come straight from the FLTLOADS `envelope.vn` point (`Nz = −NZ`, `Nx = −DX/W`) when not given explicitly; plus `Project.geometry.<surface>`.
...
- **Reads:** `Project.wing_mass`, `Project.geometry.<surface>`, `Project.aero.<surface>` (and `Project.envelope.vn` for the per-case CL/V/Nz/Nx); the inertia half reads the case's mass state exactly as WINGINER does (design note 63 D-63.1/D-63.6 — one shape, one fold per case, so the two modules' inertia for one case list is one number). Combines the AIRLOADS air-load distribution and the WINGINER inertia distribution.
...
- **Notes:** A primary structural deliverable (root shear/BM/torsion), wing only, full fidelity (all of Fx/Fz/Sx/Sz/Mxx/Myy/Mzz). SELECT selects the governing cases — since #292 each slot's net-governing run over the FLIGHT mass states (design note 63 D-63.7; the slot × case variant table is `wing_variants`, printed in the report's §3.2) — and NETLOADS runs the delivered set; an entered `wing_mass.cases` list filters which slots run and never re-points them.
...
- **Notes:** **Standalone** — no module inputs/outputs (UG Table 2.2); all data is direct input. Already supports measured-vs-approximated rotating inertia and SI/Imperial. Serves as the reference template for every other module's calc/units/report/CSV pattern. **The GUI is multi-engine:** the engine-mount page exposes the first-class multi-engine `Project` — the layout (`SINGLE_NOSE`/`TWIN_WING`/`QUAD_WING`) sets the engine count and each engine's inputs are entered against its own record. Results and exports cover every engine. A single engine reduces exactly to `run_all`. **Each engine's conditions are tagged with an identity that is unique in the installation** (`engine_tags`, #241): the designation alone where it already separates them — nothing renames on a mixed installation — and the designation plus the side, read off the engine's own butt line, where two engines share one, which is the Baron's case (two `CONTINENTAL IO-550-C`, six conditions, three titles). The tag is applied **before** the `CaseRef` is minted, so the load-case index and the applied CSV say which engine `EM-04` is instead of repeating `EM-01`'s description.
...
- **Every entered engine is failed, and every recovered case is a fin design
  ...
  fixture set** — 1.6x `baron_58`'s largest SELECT case, 2.6x `atr42_100`'s
  (3.3x on the since-retired `dhc8_dash8`, #264) — and until this step the fin
  was sized without it. The
...
- **Writes:** derived MAC / XLEMAC / Y_MAC / AR / span (obtained by running the
  ...
  the D-5 decision in `docs/25_notes/05_phase_d_gui_workflow_plan.md`). Keys: `wing`
...
- **Reads:** SELECT's fuselage critical conditions via `select.select_fuselage(project)`
  ...
  **since design note 63 D-63.8 (#289) the derived table is per condition**: each
  fuselage condition's V-n point names its CG case, that case's loading — entered
  or searched — is the mass state, and `mass_distribution.fuselage_beam_stations(project,
  loading)` lumps that loading's body parts, so a 2,063 lb airplane's beam carries
  1,733 lb of body mass and not the database's 3,070; the entered override table
  stays one per project and `fuselage_override_varies_by_case` says when that
  cannot follow the cases; a loading the search cannot produce falls back to the
  whole database with the reason in `BodyLoadResult.mass_state`)
...
- **Notes:** the inertia sign is **d'Alembert** (`−n·W_surf`), set by the case's
  ...
  `geometry.surfaces` is validated against the **entered** scalars on
  ...
  since #25 step 2 (note 54 D-54.1) a **blank** scalar derives from the
  polylines instead (`tail_geometry.boundary_derived_scalars`), and the
  validator's comparison retires field-by-field exactly as the scalars stop
  being typed —
...
- **Source:** `sloads/gear_loads.py`. Decision **G-12** of
  [`../25_notes/23_step10_ground_cases_plan.md`](../25_notes/23_step10_ground_cases_plan.md).
...
- **Source:** `sloads/modules/balance/` (the package of #191; its `__init__.py`
  ...
  ([`../25_notes/18_b8a_lateral_closure_plan.md`](../25_notes/18_b8a_lateral_closure_plan.md),
...
- **Every delivered row states which case it is (#241, 2026-09-13).** The file
  ...
  copied off the case's own `CaseRef` by the one owner,
  `applied.case_identity`, so no producer decides for itself what its rows say
  about their case. A delivered id that the index lists under a shorter name —
  a handed twin `W-05R`, or the 23.371(b) gyro condition's four sign
  combinations `EM-06a…d`, which one `ConditionResult` cannot carry four
  `CaseRef`s for — joins through `case_ids.index_case_id`, the one place that
...
- **T-tail transfer (step T7, 2026-08-13).** Gated on
  ...
  **last** `GRID` (no new node): the balancing tail load at that case's own V-n
  ...
  T-tail fixtures (`atr42_100`, `concept_regional_jet`) declare `t_tail` since
  that date.
...
- **Fuselage beam mass (step B1, 2026-08-08).** `body_loads` integrates the station
  ...
  share to the wing and puts only the remainder on the beam, so the same pounds
  never ride both beams. **Since design note 63 (#289)** the wing's mass *is* the
  item database — WINGINER's panel is derived from the `PANEL` rows and its
  concentrated masses are the `POINT` rows of the case's loading, so there is
  no second list to tie against; what remains is the panel override against the
  derived value (`wing_panel_override_open`) and the per-case identity
  `mass_distribution.wing_state_tie` (G-63.1). See `CONVENTIONS.md` §1 and
...
- **The per-case factor, stated not applied (defect M4-7, then note 49 OR-116):**
  ...
  **That case's** `safety_factor` (default `constants.ULTIMATE_FACTOR` = 1.5 per
  ...
  `deck_format.case_sf()` — never a flat suite-wide constant — and is **stated**:
  every CSV carries it in the last column (`SF`), and every card block states it
  through `deck_format.basis_sentence`, the single owner of that wording, per
...
- **The linkage in the deliverables (design note 17, 2026-08-13).** The case id
  ...
  **two** deck numbers, and every document therefore states **both, qualified by
  ...
  `::test_the_shipped_case_index_states_the_same_pairs_as_the_csv`.
...
- **Source:** `sloads/export/lra_model.py` (export) and
  ...
  `docs/25_notes/24_lra_beam_model_review_note.md` (target F1–F8, decisions
  BM-1…BM-5, agreed 2026-08-15) and
  `docs/25_notes/27_lra_model_implementation_note.md` (LM-1…LM-7).
...
### Workbook export bridge — RETIRED (Step D8.2; deleted at #270)
- Was `sloads/export/workbook.py::build_workbook`, with an `openpyxl`
  dependency: a pure re-shaping of the Export page's already-computed CSV rows
  into one multi-sheet `.xlsx` — `Project`, one sheet per module with results,
  `Case Index`, and the tabular sbeam artifacts.
- **Deleted at #270** (note 57 D-57.6 as amended 2026-09-11). The Export page
  was its only consumer, the GUI that survives excludes it by charter, no CLI
  target ever existed, and the owner ruled it unused. The issue package's
  `data/` (#245) is the single tabular channel and **no `data/`-derived
  workbook replacement is built**. Recorded here rather than deleted outright
  because a reader of this document should find out what happened to a bridge
  it used to specify.

### Summary report — RETIRED, its cross-cutting sections merged (Step G8, #278/#270)
- **The two documents became one (design note 60 D-60.7…D-60.11, 2026-09-13).**
  `app/views/export_report.py` was the front-end consumer of
  `content.build_report`, so note 57 D-57.1's deletion of `app/views/` retired
  this document. It was **not the only** consumer, as note 60 and #278's backlog
  row both said: `cli.py --report` rendered the same document headless and no
  test drove it, so the claim went unchallenged until #270 deleted the builder
  and nothing went red. That flag now renders the surviving document, and
  `tests/test_cli.py` holds it to the reachability rule the export menu has
  always been held to. Four cross-cutting assets merged into the oracle report ahead
  of that, as front matter after the introduction
  (`oracle_content.FRONT_SECTIONS`, so the section plan stays derived and
  G-OR-2 survived the re-cut on the OR-16 pattern): **axes and sign
  ...
  matrix**, and the **bundle manifest**. One builder served both documents
  while both existed (`sloads/report/front_sections.py`; practice 3), so the
  merge could not drift from what it merged.
- **Deleted at #270**, after the merge and never before it: `build_report` and
  its nine section builders, `latex.render_document`/`render_report` and the
  title page and running heads with them, `sloads/report/bundle.py` (the Export
  zip's member list, whose only consumer was the page), and the tests that
  drove them. What remains of `content.py` is the content model — `Section`,
  `Table`, `Figure`, `PlotData`, `Units`, `component_loads` — and the three
  plot-data producers the surviving document and the GUI both draw from.
- **Every section is accounted for.** Each key of the retired document's
  section list is declared merged or superseded, with its successor or its
  reason, in `front_sections.SUMMARY_DISPOSITION`, audited against
  `front_sections.RETIRED_SUMMARY_SECTIONS` by `tests/test_front_sections.py`
  (note 60 gates 11 and 12). Two required a decision rather than a
  presumption: the approved-corrections table (it describes the tool, and
  reaches a reader through the methods statement in every shipped file's
  header) and the balanced free-free cases (the surviving document's section
  set is derived from the oracle step set, which `balanced_cases` is not in;
  each case's pre-closure residual and closure are written in band on the deck
  that carries it, and Appendix G carries the lumping cost).
- **The successor** is the technical report:
  [`ORACLE_REPORT.md`](ORACLE_REPORT.md), built by
  `sloads/report/oracle_content.py` + `oracle_latex.py` over `content.py`'s
  model and `latex.py`'s emitters, written as an issue package by
  `sloads/export/report_package.py`, and compiled by `sloads/export/pdf.py`.
  Headless: `cli.py --report out.tex|out.pdf [--units …] [--generated …]`,
  which renders the document alone — the package is the Report page's.
- **Its content standard,** [`SUMMARY_REPORT.md`](SUMMARY_REPORT.md), is kept
  as the record of what the document required; `ORACLE_REPORT.md` is the
  normative one now and inherits §3.5's one-system rule under OR-5.
...
| `wing_mass` (the panel *shape*: density ratio, inboard rib, surface, the case list, `panel_weight_override_lb`) | Wing Loads page | WINGINER, NETLOADS, `balance` — the mass itself is `weight.items` since note 63 |
...
| Merged page (nav step) | Display groups (sections of the one generic renderer since #270 — `st.tabs` in the retired `app/` front-end) | Folded calc modules (still registered/tested) |
|---|---|---|
...
00_backlog.md` Step D1 (moved to `90_record/00_completed_development.md` once
...
- **One ID per physical condition (M4-2).** Where two modules deliver the same
  ...
  different V-n point (`tests/test_wing_case_derivation.py` enters a `PHAA`
  at 170 kt on `atr42_100` against SELECT's 147.7 kt; the shipped table is
  empty since #260). Only the speed moves: `case_id`, CG, altitude and the FAR
  ...
  (PHAA 1, PLAA 2, PMAA 3, NMAA 4, ACRL 5, TORS 6; NHAA 7, NLAA 8, PNZ 9,
  NNZ 10 since design note 62 D-62.3), not of its position in any
  list — so a missing pick, or an **empty** slot (note 62 D-62.2: no eligible
  point, or a load-factor-extreme point another slot already delivers), leaves
  a gap instead of renumbering its neighbours,
