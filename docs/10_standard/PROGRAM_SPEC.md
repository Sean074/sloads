...
- **The wing search above `SELECT.BAS` (design note 62, #288, 2026-09-17).** The `.BAS` searches three positive angle-of-attack slots and **one** negative; the negative side now carries the same triad — **NHAA** (largest resultant among STALL −N / STALL −1G), **NMAA** narrowed to the VC pair (MAN −C / GUST −C; its Appendix A pick is unchanged), **NLAA** (MAN −D / GUST −D) — and a load-factor-extreme pair, **PNZ** (largest `nz` over every positive-family label, tie-break largest resultant) and **NNZ** (most negative `nz` over every negative-family label). A negative slot admits a candidate only when the **wing's** lift is negative (`LZW < 0`) and PNZ only when it is positive — the sign tested is the wing's, not the airplane's load factor (D-62.2) — and a slot with no eligible point is **empty**, a gap in the W- band. A PNZ/NNZ point that is already another slot's pick is not delivered twice (D-62.8's coincidence rule, `case_ids` M4-2 decision 1). All four join `SYMMETRIC_WING_CONDITIONS`, so the LRA deck gains up to four symmetric subcases closing under G-OR-72; a slot whose CG case has no derivable loading is a `loading-not-derivable` skip like the six before it (`baron_58` W-07/W-08/W-09 at "fwd gross"/"fwd regardless", until #290). The six Appendix A picks reproduce unchanged (gate 1); the new slots are closure-gated on every fixture's frozen pick (`tests/test_select.py` G-62.1/G-62.2, `test_balance.py` G-62.3, `test_case_ids.py` G-62.4). Fixtures' hand-entered `wing_mass.cases` are untouched, so no new slot reaches WINGINER/NETLOADS until note 63 (#289).
