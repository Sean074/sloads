...
- **The boundary-derived seam is marked (note 54 D-54.1 / #25 step 1, 2026-09-10):** the planform-geometry scalars of `TailLoadsInput` (9 of 17) and `VTailLoadsInput` (10 of 15) — every area, span, MAC, MAC station and aspect ratio — are marked `[D]` in the dataclasses (the mark travels into `DATA_DICTIONARY.md`) and named in `models.inputs.HTAIL_BOUNDARY_DERIVED` / `VTAIL_BOUNDARY_DERIVED` (`{field: deriving surface}`), the machine-readable membership #25 step 2's boundary-line derivation will consume; field order is untouched (a persisted shape), so the physical regrouping rides step 2's schema bump. Until step 2 they stay entered and **oracle-authoritative** (the 1 % `validate_tail_planform` guard holds the two representations together); the aero/control/mass fields stay entered permanently. Guard: `tests/test_empennage.py::test_the_boundary_derived_marking_partitions_the_tail_blocks` — a field added to either block must declare its side of the seam.
