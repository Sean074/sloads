...
| `weight.cg_cases[].loading` (the case's mass state, D-25 / note 63 D-63.1) | Payload Cases tab (the editor is #290; the JSON until then), else derived by `mass_distribution.derive_case_loadings` | WINGINER, NETLOADS (`wing_mass_state` per wing case), `body_loads` (per fuselage condition), `balance` (per assembled case), the CONM2 overlay cards |
