...
def methods_statement(
    ...
    *,
    ...
    L.append(
        ...
        f"({_ult_markers(system)}). Where a file MIXES bases -- as the "
        f"vertical tail's do, since 23.367(a)(2) is an ultimate case among "
        f"limit ones -- the columns stay plain and the per-row 'SF' cell is "
        f"the basis: a column heading must not state a basis true of only "
        f"some of its rows. The "
