...
def index_case_id(case_id: str) -> str:
    ...
    (``W-05R``) is the assembled deck's view of ``W-05``, a suffix on an id that
    *is* listed, so the join is a strip. (The gyro condition's ``a``-``d``
    suffixes retired at design note 66 Q7, #286: each sign combination carries
    an id of its own.)

    Returns ``case_id`` unchanged when it carries no hand, which is every id in
    every other family.
    """
    return unhanded_case_id(case_id)
